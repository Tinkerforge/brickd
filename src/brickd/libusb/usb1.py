...
# (version 4855d3922524732ab4df)
#
# The file is besides this header unchanged, it is licensed as GPL V2
# according to the copying file in the git.
#
# Since there are no packages for any of the libusb-1.0 python wrapper in
# Linux distributions yet, this light-weight one is pulled in directly.
#
# Unfortunately the status of user mode usb libs is a little bit muddled.
# The most used usb lib is libusb-0.1. This one is present in most linux
# distros and has bindings for most languages etc. It, however, does not
# have support for threads, which is needed by brickd.
# The author of libusb-0.1 started a libusb-1.0 version but never released it.
# So a new project named libusb-1.0 was started (which is used by this wrapper).
# Likewise, the "old" libusb-1.0 was continued by another team under the name
# openusb. Yep, that is confusing :-). Nevertheless, the new libusb-1.0 is the
# only stable usb lib with thread support and python wrappers.
#
# The more advanced python wrapper for libusb-1.0 seems to be pyusb:
# pyusb.sourceforge.net. But it not yet in linux distributions either and
# seems to be a little bit vacant.
#
# Therefore brickd uses at the moment the most lightweight wrapper that
# can be extended easily if needed. 
#
# In the future, if one of the usb libs and python wrappers wins this "war",
...
class USBTransfer(object):
    ...
    def getBuffer(self):
        ...
        if transfer.type == libusb1.LIBUSB_TRANSFER_TYPE_CONTROL:
            ...
            result = string_at(transfer.buffer, self.getActualLength())
